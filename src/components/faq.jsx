            ...
            {faqs.map((faq, index) => (
                <div key={index} className="border-b border-[#D7DBDE] py-4">
                    <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleFAQ(index)}>
                        <h3 className={`text-lg font-medium ${activeIndex === index ? "text-[#0A72F4]" : "text-black"}`}>{faq.question}</h3>
                        <span className={`text-xl md:text-2xl font-semibold transform transition-transform duration-300 ${activeIndex === index ? "text-[#0A72F4] rotate-45" : "text-black"}`}>+</span>
                    </div>
                    {activeIndex === index && (
